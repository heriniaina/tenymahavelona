...
public class ChapterViewActivity extends BaseActivity implements IScrollListener {
	...
	public void onCreate(Bundle savedInstanceState) {
	    ...
	    if(extras == null) {
	        ...
	        this.book = (Book) extras.getSerializable("com.baiboly.katolika.Book");
	...
	private void setupToolbar() {
        ...
        if(this.chapterId < this.book.getChapters() || this.book.getId() < 73 || (this.book.getId() == 73 && this.chapterId < 22)) {
        ...
        public void run() {
			...
			if(pref.getSecLanguage() == Preference.LANG_NONE) {
				if(verseIds == null) {
					...
					cursor = adapter.fetchVerses(book.getId(), chapterId, table, verseIds);
				}
			}
			else {
				...
				if(verseIds == null) {
					...
					cursor = adapter.fetchVerses(book.getId(), chapterId, table, verseIds);
					cursorSec = adapter.fetchVerses(book.getId(), chapterId, table2, verseIds);
				}
			}
	...
	{
            startActivity(new Intent(ChapterViewActivity.this, SearchViewActivity.class));
	...
	private String getSelectedVerseText() {
		...
		if(pref.getSecLanguage() == Preference.LANG_NONE) {
			cursor = adapter.fetchVerses(book.getId(), chapterId, table, selectedVerses, 'P');
		}
		else {
			String table2 = "verses";
			switch(pref.getSecLanguage()) {
			...
			cursor = adapter.fetchVerses(book.getId(), chapterId, table, selectedVerses, 'P');
			cursorSec = adapter.fetchVerses(book.getId(), chapterId, table2, selectedVerses, 'S');
		}
		...
		if(cursorSec != null && !cursorSec.isClosed()) {
			...
			while(!cursorSec.isAfterLast()) {
				if(cursorSec.getInt(0) > 0) {
					sb.append(chapterId).append(":").append(cursorSec.getInt(0)).append(" ");
				}
